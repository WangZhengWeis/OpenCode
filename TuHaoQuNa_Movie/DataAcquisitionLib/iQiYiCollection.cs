...
namespace DataAcquisitionLib
{
    public class iQiYiCollection
    {
        ...
        public void CollectioniQiYiHtmlMovie(int movieID, string name, int timeout = 30000)
        {
            var res_str = "";
            CookieCollection cookie = new CookieCollection();
            var url = "http://so.iqiyi.com/so/q_" + name;
            try
            {
        ...
        public List<Model.Player> AnalysisPlayer(string html)
        {
            try
            {
                ...
                hd.LoadHtml(html);
                var movie_nodes = hd.DocumentNode.SelectNodes("//ul[@class='mod_result_list']/li[@class='list_item'][@data-widget-searchlist-catageory='电影']");
                if (movie_nodes!=null)
                {
                    foreach (var item in movie_nodes)
                    {
                        var a_node = item.SelectSingleNode("./a");
                        var player_url = a_node.GetAttributeValue("href", "").Trim();
                        string vid = item.GetAttributeValue("data-widget-searchlist-tvid", "").Trim();
                        if (!string.IsNullOrWhiteSpace(player_url)&&player_url.Contains("http://www.iqiyi.com/")&& !string.IsNullOrWhiteSpace(vid) && vid != "0")
                        {
                            string poster = null;
                            var poster_node = a_node.SelectSingleNode("./img");
                            if (poster_node!=null)
                                poster = poster_node.GetAttributeValue("src", null);

                            string power = null;
                            var power_node = a_node.SelectSingleNode("./p");
                            if (power_node != null)
                                power = power_node.InnerText.Trim();

                            list.Add(new Model.Player()
                            {
                                Category = item.GetAttributeValue("data-widget-searchlist-catageory", null),
                                CreateTime = DateTime.Now,
                                Episodes = null,
                                LastTime = DateTime.Now,
                                Name = item.GetAttributeValue("data-widget-searchlist-tvname", null),
                                Platform = "iqiyi",
                                VID = vid,
                                PlayerUrl = player_url,
                                Poster = poster,
                                Power = string.IsNullOrWhiteSpace(power)?null: power
                            });
                        }
        ...
        public void CollectioniQiYiHtmlTv(int movieID, string name, int timeout = 30000)
        {
            var res_str = "";
            CookieCollection cookie = new CookieCollection();
            var url = "http://so.iqiyi.com/so/q_" + name;
                    ...
                    foreach (var item in movie_nodes)
                    {
                        ...
                        if (play_node != null)
                        {
                            ...
                            if (play_url.Contains("http://v.youku.com/v_show/id_"))
                            {
                                ...
                                model.Power = null;
                                var power_node = item.SelectSingleNode("./div[@class='s_poster']/div[@class='s_target']/span[@class='tip_bk']");
                                if (power_node != null)
                                {
